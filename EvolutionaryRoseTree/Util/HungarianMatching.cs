...
{
    class HungarianMatchingHelper
    {
        public static void GetMinimumWeightMatchingCost(double[,] costMatrix,
            out int[] result, out double cost)
        {
            var matching = new HungarianMatching(costMatrix);
            result = matching.execute();
            cost = 0.0;
    ...
    public class HungarianMatching
    {
        ...
        public int[] execute()
        {
            ...
            int[] result = copyOf(matchJobByWorker, rows);

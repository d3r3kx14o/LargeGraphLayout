...
namespace RoseTreeTaxonomy.Algorithms
{
    public class SpillTree_Mine
    {
        ...
        public int stop_partition_nodes_number = 0;

        public bool bChooseMedium = true;

        public SpillTree_Mine(int k, int projectdimension, double tau)
        {
            this.k = k;
            this.projectdimension = projectdimension;
            this.tau = tau;
            this.stop_partition_nodes_number = (int)Math.Ceiling(this.k / 0.3); 
        }
        ...
        {
            if (node.points.Count <= stop_partition_nodes_number)
            {
            ...
            double[] mid_point;
            mid_point = RoseTreeMath.ProjectData_MidPoint(node.lpv, node.rpv, this.projectdimension);
            ...
            //if (bChooseMedium)
            {
                overlapping = Partition(node, node.points, out leftchild_pointlist, out rightchild_pointlist);
        ...
        private bool Partition(SpillTreeNode node, List<RoseTreeNode> pointlist, out List<RoseTreeNode> leftchild_pointlist, out List<RoseTreeNode> rightchild_pointlist)
        {
            leftchild_pointlist = new List<RoseTreeNode>();
            rightchild_pointlist = new List<RoseTreeNode>();

            int pointCnt = pointlist.Count;
            double[] pointProjectionLength = new double[pointCnt];
            double[] rpv = node.rpv;

            //normalize half u
            double[] half_u = node.half_u;
            double normsum = 0;
            for (int j = 0; j < projectdimension; j++)
                normsum += half_u[j] * half_u[j];
            double norm = Math.Sqrt(normsum);
            for (int j = 0; j < projectdimension; j++)
                half_u[j] /= norm;

            //Calculate projection length of each point
            for (int i = 0; i < pointCnt; i++)
            {
                double projectLen = 0;
                double[] point = pointlist[i].projectdata;
                for (int j = 0; j < this.projectdimension; j++)
                    projectLen += (point[j] - rpv[j]) * half_u[j];   //Dot_Product(point-rpv,n_half_u) 
                //double[] minus = RoseTreeMath.ProjectData_Minus(point, rpv, projectdimension);
                //double projectLen2 = RoseTreeMath.ProjectData_DotProd(minus, normalized_half_u, projectdimension);
                //if (Math.Abs(projectLen - projectLen2) > 1e-8)
                //    Console.WriteLine();
                pointProjectionLength[i] = projectLen;
            }

            int rcutpos = (int)Math.Ceiling(0.3 * pointCnt);
            int lcutpos = (int)Math.Floor(0.7 * pointCnt);

            double rcutprojlen = RoseTreeMath.GetDLargestValue(pointProjectionLength, rcutpos);
            double lcutprojlen = RoseTreeMath.GetDLargestValue(pointProjectionLength, lcutpos);


            //Update center to be the average of rcut and lcut
            double midpointprojlen = (rcutprojlen + lcutprojlen) / 2;
            double[] center = node.center;
            for (int j = 0; j < projectdimension; j++)
                center[j] = rpv[j] + half_u[j] * midpointprojlen;  //rpv + n_half_u * len

            bool overlap;
            //test if we have to turn to non_overlapping partition
            if (lcutprojlen - rcutprojlen >= tau)
            {
                lcutprojlen = midpointprojlen + tau / 2;
                rcutprojlen = midpointprojlen - tau / 2;
                for (int i = 0; i < pointCnt; i++)
                {
                    if (pointProjectionLength[i] <= lcutprojlen)
                        rightchild_pointlist.Add(pointlist[i]);
                    if (pointProjectionLength[i] >= rcutprojlen)
            ...
            else
            {
                for (int i = 0; i < pointCnt; i++)
                {
                    if (pointProjectionLength[i] < midpointprojlen)
                        rightchild_pointlist.Add(pointlist[i]);
                    else
                        leftchild_pointlist.Add(pointlist[i]);
                }
            ...
            node.lcut_projlen = lcutprojlen;
            node.rcut_projlen = rcutprojlen;
            node.center_projlen = midpointprojlen;
            return overlap;
        }
        ...
        {
            node.InsertNode(newpoint);
            if (node.left_child == null && node.right_child == null)
            {
                //node.points.Add(newpoint);//Xiting, radius should be updated
                newpoint.spilltree_positions.Add(node);
                return;
            }
            else
            {
                List<RoseTreeNode> newpoint_list = new List<RoseTreeNode>();
                //List<RoseTreeNode> leftpoint_list, rightpoint_list;
                newpoint_list.Add(newpoint);

                //if (bChooseMedium)
                {
                    double projlen = 0;
                    double[] point = newpoint.projectdata;
                    double[] rpv = node.rpv;
                    double[] norm_half_u = node.half_u;
                    for (int j = 0; j < projectdimension; j++)
                        projlen += (point[j] - rpv[j]) * norm_half_u[j];
                    if (node.overlapping)
                    {
                        if (projlen >= node.rcut_projlen)
                            RecursiveInsert(newpoint, node.left_child);
                        if (projlen <= node.lcut_projlen)
                            RecursiveInsert(newpoint, node.right_child);
                    }
                    else
                    {
                        if (projlen >= node.center_projlen)
                            RecursiveInsert(newpoint, node.left_child);
                        else
                            RecursiveInsert(newpoint, node.right_child);
                    }
